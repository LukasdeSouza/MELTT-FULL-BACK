...
const CustosFixosPage = () => {
  ...
  const fetchCustosFixo = async (searchTerm = '', turmaId = '', dataVencimento = '') => {
    let url = `/custos?limit=all&tipo_custo=Fixo`;
  ...
  const handleEditarSituacao = (custo: Custos) => {
    ...
    console.log("Nova situação:", custo.situacao);
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      fetchCustosFixo(filtro, filtroTurma, filtroData);
    }, 500);
